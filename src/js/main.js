...
if (typeof weatherStream == undefined) {
  ...
  const weatherStream = streamApi.sourceEvent("se_weatherStream", "demo_weather_source").subscribe(data => {
  ...
  const sensorStream = streamApi.timedQuery("tq_sensorStream", "Sensor", 10, whereParam, null, null).subscribe(updateSensorData);
  const sensor1EventStream = streamApi.clientEvent("ce_sensor1EventStream").subscribe(data => {
    $("#sensor1_value_min").text(data.min);
    $("#sensor1_value_max").text(data.max);
  });
  const sensor2EventStream = streamApi.clientEvent("ce_sensor2EventStream").subscribe(data => {
    $("#sensor2_value_min").text(data.min);
    $("#sensor2_value_max").text(data.max);
  });
  const sensorUpdateEventStream = streamApi.topicEvent("te_sensorUpdateEventStream", "/mt/sdk_stream_demo/sensor").subscribe(data => {
    if (data && data.id === 3) {
  ...
  function unsubscribeAll() {
    weatherStream.unsubscribe();
    sensorStream.unsubscribe();
    sensor1EventStream.unsubscribe();
    sensor2EventStream.unsubscribe();
    sensorUpdateEventStream.unsubscribe();
  }
